...
export class FileSystemReader implements Reader {
  ...
  public readAnyOf(filenames: string[]): string | undefined | ReaderFileLackPermissionsError {
    ...
    for (let id = 0; id < filenames.length; id++) {
      ...
      try {
        ...
        const readErr = error as NodeJS.ErrnoException; // 使用类型断言

        if (!firstFilePathFoundButWithInsufficientPermissions && typeof readErr?.code === 'string') {
